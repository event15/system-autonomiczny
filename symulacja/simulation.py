...
def stworz_bodzce(wektor_czasu):
    lista_bodzcow = [
        {"start": 1.0, "stop": 12.0, "offset": 2.8, "amplitude_sin": 0.3, "frequency_sin": 0.0},
...
def uruchom_symulacje():
    ...
    korelator = Korelator(przewodnosc_poczatkowa_g0=PRZEWODNOSC_POCZATKOWA_G0, alfa_g=ALFA_G_PRZEWODNOSCI * 5, # Jeszcze bardziej zwiększamy alfa_g
                              ekstynkcja_r=EKSTYNKCJA_REJESTRACJI_R * 5, # Jeszcze bardziej zwiększamy ekstynkcja_r
                              ekstynkcja_d=EKSTYNKCJA_DEREJESTRACJI_D * 5, # Jeszcze bardziej zwiększamy ekstynkcja_d
    ...
    wartosci_delta_g = np.zeros(liczba_krokow_czasowych) # Dodajemy tablicę na przyrost G

    czasy_decyzji = []
    czasy_reakcji = []
    zaplanowane_reakcje = []
    vk_decyzje = [] # Dodajemy listę na Vk w chwili decyzji

    poprzednia_g = PRZEWODNOSC_POCZATKOWA_G0  # Dodaj tę linię
    ...
    for i in range(liczba_krokow_czasowych):
        ...
        wartosci_delta_g[i] = abs(wartosci_g[i] - poprzednia_g) # Obliczamy przyrost G
        poprzednia_g = wartosci_g[i] # Aktualizujemy poprzednia_g dla następnego kroku czasowego
        ...
        if sygnal_decyzji:
            ...
            vk_decyzje.append(wartosci_vk[i]) # Zapisujemy Vk w chwili decyzji
...
def generuj_wykresy_wynikow_symulacji(wektor_czasu, suma_bodzcow, wartosci_vr, wartosci_vh, wartosci_ve, wartosci_g, wartosci_k,
                                    ...
                                    wartosci_myslenia, wartosci_kg_graniczne, wartosci_delta_g, vk_decyzje):

    fig = plt.figure(figsize=(18, 27)) # Zwiększamy wysokość figury, aby pomieścić nowy subplot
    kolory = {
        'bodziec': '#8A2BE2',
        'vr': '#4169E1',
        'vh': '#228B22',
    ...
    ax1 = fig.add_subplot(4, 3, 1) # Zmieniamy układ na 3x3
    ...
    ax2 = fig.add_subplot(4, 3, 2) # Zmieniamy układ na 3x3
    ...
    ax3 = fig.add_subplot(4, 3, 3) # Zmieniamy układ na 3x3
    ...
    ax4 = fig.add_subplot(4, 3, 4) # Zmieniamy układ na 4 wiersze
    ...
    ax5 = fig.add_subplot(4, 3, 5) # Zmieniamy układ na 4 wiersze
    ...
    ax6 = fig.add_subplot(4, 3, 6) # Zmieniamy układ na 4 wiersze
    ...
    ax10 = fig.add_subplot(4, 3, 10)  # Dodajemy subplot J, indeks 9
    ...
    ax10.set_title('J) Zależność Ve(Vk) z Asymptotą', fontsize=9, pad=6)  # Tytuł subplotu J
    ax10.axhline(PROG_DECYZYJNY_VD, color='black', linestyle='-.', label=f'Próg Vd')  # Dodajemy próg Vd

    # Subplot K: Zależność Ve(t) z progiem Vd
    ax11 = fig.add_subplot(4, 3, 11)  # Dodajemy subplot K, indeks 11
    ax11.plot(wektor_czasu, wartosci_ve, color=kolory['Ve'], lw=1.0)
    ax11.axhline(PROG_DECYZYJNY_VD, color='black', linestyle='-.', label=f'Próg Vd')  # Dodajemy próg Vd
    ...
    ax11.set_title('K) Zależność Ve(t) z Progiem Vd', fontsize=9, pad=6)  # Tytuł subplotu K

    # Subplot L: Zależność Ve(Vk) z progiem Vd (alternatywa)
    ax12 = fig.add_subplot(4, 3, 12)  # Dodajemy subplot L, indeks 12
    ax12.scatter(wartosci_vk, wartosci_ve, color=kolory['Ve'], alpha=0.8)
    ax12.axhline(PROG_DECYZYJNY_VD, color='black', linestyle='-.', label=f'Próg Vd')  # Dodajemy próg Vd
    ...
    ax12.set_title('L) Zależność Ve(Vk) z Progiem Vd (Alternatywa)', fontsize=9, pad=6)  # Tytuł subplotu L

    plt.tight_layout(pad=2.0)
    plt.subplots_adjust(hspace=0.6, wspace=0.3, bottom=0.05)
    plt.show()
